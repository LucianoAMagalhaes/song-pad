...
- **Fase:** MVP (Fase 1) completo. **Fase 3 (cloud) completa** (sessão 2026-05-30): login Google, Firestore (schema + regras + repositórios real-time/offline), migração de dados locais e deploy em https://song-pad-app.web.app. A Fase 2 (pós-MVP) continua adiada por opção do utilizador. Próximos rumos possíveis: itens da Fase 2, ou partilha/bandas e empacotamento Capacitor da Fase 3.
...
- **Troca de acordes (variações):** Sim — tocar num acorde, **na letra (acima da sílaba) ou num diagrama do painel**, abre um popup ancorado "Trocar X (toda a música) por:" com variações da **mesma fundamental** (Am → Am7, Am9, Amaj7, Asus4…). UI partilhada em `ChordSwapTrigger` (`src/components/ChordSwapTrigger.tsx`): popup via `createPortal` com posição `fixed` (escapa ao `overflow-x-auto` da cifra), fecha em clique-fora/scroll. O `ChordRenderer` torna os acordes clicáveis quando recebe `originalContent`+`onSwapChord` (mapeia display→original por alinhamento de segmentos); o `ChordPanel` envolve cada diagrama no mesmo trigger. Lógica pura em `src/lib/chordSwap.ts` (`parseChord`, `chordVariants` via `COMMON_SUFFIXES`, `withSuffix`, `replaceChord` literal por token `[chord]`). A troca **persiste** na música (todas as ocorrências) via `songRepository.update` com atualização otimista no `SongView` e no `SetlistPlayer`. Como só muda o sufixo (mantém a fundamental), funciona em conjunto com a transposição (aplica-se ao conteúdo original; nomes das variações mostrados já transpostos). Testes em `src/__tests__/chordSwap.test.ts`. Verificado visualmente.
- **Diagramas de acordes (violão):** Sim — `ChordPanel` (`src/components/ChordPanel.tsx`) mostra uma faixa colapsável com o desenho do braço de cada acorde único da música (componente SVG `ChordDiagram` com cordas soltas/abafadas, números dos dedos, pestanas e base-fret). Dados embutidos via `@tombatossals/chords-db` (`src/lib/guitarChords.ts`, lookup puro: normaliza enarmónicos, mapeia sufixos comuns, resolve acordes com baixo `/` e faz fallback à tríade) — funciona **offline**. Acompanha a transposição (recebe o conteúdo já transposto). Estado aberto/fechado lembrado em `localStorage` (`songpad:show-chords`). Integrado no visualizador (`SongView`) e no player de palco (`SetlistPlayer`). Helper `extractChords` no `chordProParser`. Testes em `src/__tests__/guitarChords.test.ts`. Verificado visualmente (screenshot dos diagramas).
...
- **Deploy (Firebase Hosting):** Sim — **app no ar em https://song-pad-app.web.app** 🎉. Secção `hosting` no `firebase.json` (`public: "out"`, `cleanUrls: true`, rewrites das rotas dinâmicas → cascas `_.html`). Script `npm run deploy` (`build && firebase deploy`). Export estático condicional ao build de produção (`next.config.ts`), por isso `next dev` corre normal. Verificado ao vivo: rotas estáticas e rewrites dinâmicos devolvem 200; `/login` renderiza. Login Google funciona no domínio (Firebase autoriza `*.web.app`/`*.firebaseapp.com` por defeito).
- **Última branch trabalhada:** `feat/chord-diagrams`
- **Último PR merged:** #21 (`fix/route-id-static-export`)
...
- [x] Configurar Firebase Hosting + deploy ✅ **no ar em https://song-pad-app.web.app**; `firebase.json` com hosting + rewrites, `npm run deploy`
